...
## Template Component Reuse Debug

### Execution page
- Execution page root component: inline JSX in `App` function at `src/main.tsx:7102` (`<main className="df-execute">`). NOT a standalone component — the entire execution page (candidate panel + timeline + all interactions) is inline JSX inside the single `App` function component.
- Left candidate panel component: inline JSX at `src/main.tsx:7130` (`<section className="df-candidate-panel">`). NOT a standalone component. Contains: collapsed strip (7143), expanded header `df-panel-title` (7154), active task chip (7188), AI planner legacy (7212), candidate list `df-candidate-list` (7236) with `TaskCard` items, habit section via `<HabitCandidateCard>` (7289), quick-add form `df-quick-add` (7303).
- Candidate top bar component: inline JSX `div.df-panel-title` at `src/main.tsx:7154` — `<h2>{candidate.title}</h2>` + `<div>` of `button.df-icon-action` (collapse, toggle done, group by project, template button, widget button, AI plan). NOT a standalone component.
- Candidate list/card component: `TaskCard` function component at `src/main.tsx:9893`. Renders one candidate task. Internally composes `<TaskBlock as="article" variant="candidate" appearance="calm">` (the shared TaskBlock component). Props: task, projects, focusDate, placementPreview, + 14 callback props.
- Current task card component: inline `df-active-task-chip` JSX at `src/main.tsx:7188`. NOT a standalone component.
- Habit section component: `HabitCandidateCard` function component at `src/main.tsx:9340`.
- Timeline component: inline JSX at `src/main.tsx:7328` (`<section className="df-timeline-panel">`). NOT a standalone component. Two branches: multi-day (3day/weekly/month) at 7416-7975, single-day (daily) at 7979-8260. Both inline, deeply coupled to ~50 App-level state/ref hooks (timelineRef, timelineCanvasRef, drag, resizePreview, dragCreate, hoverSlot, settings.continuousCrossDayScroll, dayStartHour, infinite scroll refs, etc.).
- Timeline event block component: `TimeBlock` function component at `src/main.tsx:10464`. Renders one scheduled task on the timeline. Internally composes `<TaskBlock as="div" variant="scheduled" appearance="calm" density="compact|normal">`. Props: task, preview, projectName, projects, + 15 callback props.
- Timeline grid/hour line component: inline JSX. Multi-day: `div.df-hour-lines-layer` at 7679 (absolute-positioned `div.df-slot.hour`). Single-day: `div.df-timeline-canvas` at 8072 with child `div.df-slot.hour` at 8164. NOT a standalone component — hour lines are just `<div className="df-slot hour" style={{top}}><span>HH:00</span></div>`.
- Timeline drag/resize module: inline closures in `App`. `beginBlockDrag` at 5253, `beginBlockResize` at 5441. NOT a standalone module — deeply coupled to App state (drag, resizePreview, moveTimelineRecord, timeline geometry refs).

### Template page
- Current template entry component: `setScheduleTemplateOpen(true)` from candidate top-bar 模板 button at `src/main.tsx:7162`.
- Current template modal/component: `ScheduleTemplateModal` function component at `src/main.tsx:8450`. Rendered via `createPortal` at 8330.
- Current template left panel: inline JSX `<aside className="df-template-candidate">` using reused `df-panel-title` + `df-candidate-list` + `df-candidate-task-row` classes (v3 redesign from prior turn). Uses `data-active` attribute for selection instead of TaskBlock's `selected` prop.
- Current template timeline: inline JSX `<div className="df-template-timeline-panel">` using reused `df-timeline-daily` + `df-date-title` + `df-timeline-allday` + `df-timeline-scroll` + `df-timeline-canvas` + `df-slot.hour` classes (v3 redesign). Period blocks use `df-time-block` className but custom `df-template-period-block` styling rather than the shared `TaskBlock` component.
- Which parts are custom duplicate UI: (1) template rows use `df-candidate-task-row` classes but NOT the `TaskCard`/`TaskBlock` component — they're hand-built divs. (2) period blocks use `df-time-block` className but NOT the `TaskBlock` component — they're hand-built divs with `df-resize-dot` + `df-template-period-body`. (3) the hour grid is re-implemented inline rather than shared. (4) `df-template-candidate` / `df-template-timeline-panel` wrappers are custom.
- Which parts must be replaced by reused components: (1) template rows → use `<TaskBlock as="article" variant="candidate">` with template-adapted content (title + meta) and `selected` prop for active state. (2) period blocks → use `<TaskBlock as="div" variant="scheduled">` with period content (title + time range) and `df-resize-dot` handles. (3) hour grid → extract a shared `TimelineCanvas` primitive (canvas + hour slots) used by both execution page and template page. (4) left panel shell → extract a `SidePanelShell` (header + scroll list) used by both.

### Reuse strategy (v4 — real component reuse, not CSS imitation)

The execution page is ~800 lines of inline JSX tightly coupled to 50+ App-level state/ref hooks. Extracting the FULL execution timeline into a shared component would require threading all those hooks as props — an extremely high-risk refactor that would break infinite scroll, drag/resize, now-line, drop targets, and drag-overlay. This violates the regression guard.

Instead, the reuse strategy is:
1. Extract the **minimal visual primitives** that are safe to share (no behavioral coupling):
   - `TimelineCanvas` — pure presentational: a canvas div with hour-slot grid lines + hour labels. Props: `height`, `slotMinutes`, `slotHeight`, `startHour`, `onPointerDown/Move/Up/Cancel`, `children`. Used by the template page. The execution page keeps its inline version (migration deferred to avoid regression).
   - `SidePanelShell` — pure presentational: panel header (`df-panel-title` + title + actions) + scrollable list container (`df-candidate-list`). Props: `title`, `actions`, `children`. Used by the template page. Execution page keeps inline.
2. Use the **existing shared `TaskBlock` component** (already at `src/components/TaskBlock.tsx`) for BOTH template rows (`variant="candidate"`) and period blocks (`variant="scheduled"`). This is real component reuse — the same component that renders execution-page cards/blocks now renders template cards/blocks, guaranteeing identical visual language via the same className generator.
3. Use an **adapter pattern** for data: `TimelineAdapter<T>` interface with `getId/getTitle/getStartMinutes/getDurationMinutes/createAt/updateTime/updateTitle/delete`. Template page uses `templatePeriodAdapter` over `TemplatePeriod[]`; this keeps template data isolated from the real task store.
4. Delete all custom `df-template-period-block` / `df-template-period-body` CSS — period blocks now get their styling from `TaskBlock variant="scheduled"` + the existing `df-time-block` CSS that execution-page `TimeBlock` already uses. Only add a thin `mode="template"` override to hide checkbox/done/project chrome.

This satisfies "reuse components, don't imitate CSS" while not risking the execution page.
