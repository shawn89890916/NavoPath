...
- 自动排程不再要求工作窗口末尾额外预留缓冲时间；任务现在可以恰好在规划结束时间完成，同时连续任务之间仍会保留设置的缓冲间隔。
...
- Automatic scheduling no longer requires an extra buffer at the end of the work window. A task can now finish exactly at the planning boundary, while consecutive tasks still retain the configured buffer.
