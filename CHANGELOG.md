...
- 规划工作区现在以与 Execute 一致的暖纸层级呈现：左侧栏专用于树、看板、矩阵、列表和指标切换，所有视图的项目与任务块使用纯白纸面、加粗且不透明的项目色条与清晰项目标记，并可关闭规划说明。右下角添加直接打开同一行、选中态明确的任务、项目、习惯切换抽屉。候选、看板、矩阵、列表与规划树共用四边空槽的拖放反馈：源卡一经拖起便离开原位，带轻斜和中性阴影的原样卡片跟随指针，目标及后续项目稳定让位。候选与规划树在拖动时会为尚无任务的项目保留可命中的完整空槽，松手即可将任务直接归入该项目。未归属和循环任务现在同样可拖动、排序与排程。
