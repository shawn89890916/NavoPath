...
### 修复
- 规划页、计时器和专注模式的默认点缀色现在跟随当前主题，不再在项目颜色缺失时回退为荧光绿；暗色规划页侧栏也会使用暗色纸面，恢复工具文字的清晰对比度。
...
### Fixed
- Planning, timer, and focus-mode fallback accents now follow the active theme instead of reverting to fluorescent lime when a project color is missing. The Planning sidebar also uses the dark paper surface in dark mode, restoring clear contrast for its tools.
