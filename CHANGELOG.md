...
### 修复
- 重构指标视图甜甜圈图外部标注为紧凑的两段式引线 + 侧标 annotation 系统：圆环外缘短径向 tick（outer+34 折点）接 42px 水平段，项目名贴在线末端外侧（右侧 start、左侧 end），字号 12px/600，引线 1.5px、22% 半透明灰（激活 36%），整体回归安静的数据图表标注风格，不再用大字标题或长平段托住文字。默认仅显示占比 ≥6% 或前 5 名项目的标签，过小扇区只在项目列表中保留、悬停时临时浮出对应标签；同侧标签做一次 Y 排序避让，位移超 36px 或越出图表安全区则隐藏，避免拥挤和飞到边缘。右侧指标摘要删除"最高投入/Top focus"一项（与圆环图和列表重复），仅保留已安排时间、未安排时间、任务数量、完成率；圆环中心悬停态移除彩色圆点，仅保留项目名、时长与百分比。hover 时仅当前扇区外径扩展、引线起点跟随，标签与折线位置稳定不重排。
...
### Fixes
- Rebuilt the Metrics donut outside annotations as a compact two-segment leader + side-label system: a short radial tick from the arc edge (outer+34 elbow) joins a 42px horizontal segment, with the project name sitting beside the line end (start anchor on the right, end anchor on the left) at 12px/600, leader stroke 1.5px at 22% semi-transparent gray (36% when active) — returning to a quiet data-chart annotation style instead of large heading-like text or long rails carrying the text. By default only segments with >=6% share or in the top 5 by share get a permanent label; smaller segments stay in the project list and surface on hover. Same-side labels get a single Y sort-and-push collision pass, and any label that would need to move more than 36px or falls outside the safe chart area is hidden to prevent crowding or edge spills. Removed the "Top focus" item from the right-side metrics summary (it duplicated the donut and project list), keeping only Planned, Unplanned, Tasks, and Done; removed the colored dot from the donut center hover state, keeping just the project name, duration, and percentage. On hover only the active segment's outer radius expands and its leader anchor follows; label and leader positions stay stable and don't reflow.
