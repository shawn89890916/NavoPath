...
- 用户直接回复“确认/执行/取消”等简短指令时，只会绑定当前最近一轮待确认 Agent 操作并真正执行或取消；请求繁忙时会保留输入，避免误关联旧操作或静默丢失确认。
...
- Short replies such as “Confirm” or “Cancel” now apply only to the latest pending Agent turn, while busy requests keep the draft intact instead of matching an old action or silently dropping the confirmation.
