...
- 新增跨桌面与移动端的 NavoPath Obsidian Bridge：可监听 Vault 内 `升学/资料` 的增量新建、修改、重命名与删除，首次只建立哈希基线，后续仅上传变化清单和有限的日程相关片段；失败会保留并补传，设备令牌通过 Obsidian SecretStorage 保存且不会进入 iCloud 插件数据。
