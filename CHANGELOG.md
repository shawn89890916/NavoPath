...
- 任务、重复实例、习惯和小组件倒计时排程跨过午夜时现在都会同步推进结束日期，包括月末和年末边界；23:50 开始的 20 分钟计划会正确结束在次日 00:10，不再因同日结束时间早于开始时间而从时间轴消失或被统计为零时长。
...
- Task, recurring-instance, habit, and widget-countdown schedules that cross midnight now advance their end date as well, including month- and year-end boundaries. A 20-minute plan starting at 23:50 correctly ends at 00:10 the next day instead of disappearing from the timeline or contributing zero planned time because its same-day end appeared earlier than its start.
