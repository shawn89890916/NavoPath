...
### 修复
- 优化竖屏时间轴的快速交互：点按空白时段会先显示清晰的 30 分钟时间段预览，并直接拉起底部短栏填写名称、查看起止时间和日期；日期标题可展开紧凑月历快速跳转。时间轴同时支持双指纵向缩放，缩放以手势中心对应的时刻为锚点，标尺、任务块、点击与拖动坐标保持同步，并继续跟随当前活动主题。
...
### Fixed
- Refined portrait timeline interactions: tapping an empty slot now shows a clear 30-minute range preview and immediately opens a compact bottom sheet for the title, time range, and date; the date heading expands into a compact monthly quick picker. The timeline also supports two-finger vertical zoom anchored to the time beneath the gesture centre, keeping rulers, blocks, taps, and drag coordinates aligned while following the active theme.
