...
- 任务、重复实例、习惯和小组件倒计时排程跨过午夜时现在都会同步推进结束日期，包括新建、拖动、缩放、修改时长、任务与日历事件互转、旧事件恢复以及月末和年末边界；移动已有跨日记录、转换或恢复跨日事件都会保留完整时长，23:50 开始的 20 分钟计划也会正确结束在次日 00:10，不再因结束日期陈旧或同日结束时间早于开始时间而从时间轴消失、跳回旧日期、缩短为 15 分钟或被统计为零时长。
