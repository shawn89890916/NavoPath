...
- 规划任务树现在会原子地移动嵌套子任务，并拒绝把子任务放入自己的后代；无效拖拽会保留原树，不再因目标节点随源子树一同移除而丢失整棵子树。
...
- Planning-tree moves now relocate nested subtasks atomically and reject placing a subtask inside one of its own descendants. Invalid drops preserve the original tree instead of losing the entire subtree when the target disappears with the source.
