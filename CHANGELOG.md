...
- 任务、重复实例、习惯和小组件倒计时排程跨过午夜时现在都会同步推进结束日期，包括新建、拖动、缩放、修改时长以及月末和年末边界；移动已有跨日记录会保留完整时长，23:50 开始的 20 分钟计划也会正确结束在次日 00:10，不再因结束日期陈旧或同日结束时间早于开始时间而从时间轴消失、跳回旧日期或被统计为零时长。
...
- Task, recurring-instance, habit, and widget-countdown schedules that cross midnight now advance their end date across creation, moving, resizing, duration edits, and month- or year-end boundaries. Moving an existing cross-day record preserves its full duration, and a 20-minute plan starting at 23:50 correctly ends at 00:10 the next day instead of disappearing, jumping back to a stale date, or contributing zero planned time.
