...
- 桌面托盘唤醒、二次启动和系统激活现在始终定位明确的主窗口，不再因窗口列表顺序误显示组件或竖屏小窗；更新状态与组件操作转发也会跳过正在销毁的渲染器，避免关闭竞态中断主进程。
...
- Desktop tray wakeups, second launches, and system activation now always target the explicitly tracked main window instead of showing a widget or portrait window according to list order. Update-state and widget-action relays also skip renderers that are being destroyed, preventing close-time races from interrupting the main process.
