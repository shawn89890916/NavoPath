...
### 修复
- 插件设置与使用教程现在准确区分可运行的官方内置插件和仅提供经校验 manifest/配置的桌面本地插件；本地项不再被描述为会加载 `index.js` 或显示“运行中”，其权限明确标记为声明信息。外部 manifest 还会拒绝对象保留键与重复配置字段，并按类型、数值范围、选项和文本长度规范化默认值及已保存配置。
