...
- 自动排程不再要求工作窗口末尾额外预留缓冲时间；完整任务可以恰好在规划结束时间完成，跨日期拆分的长任务也会使用每个窗口的全部可用容量，同时连续任务之间仍会保留设置的缓冲间隔。
...
- Automatic scheduling no longer requires an extra buffer at the end of the work window. A complete task can finish exactly at the planning boundary, and long tasks split across dates now use each window's full available capacity, while consecutive tasks still retain the configured buffer.
