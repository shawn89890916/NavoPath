...
- 小组件播放与更多图标现在在主卡片中保持垂直居中；“更多”面板加宽透明度滑条、改用当前墨水色并上移顶部工具栏，底部仅保留保存操作。
...
- Play and More now stay vertically centered in the widget card; the More panel widens its opacity slider, uses the current ink color, moves the utility row upward, and keeps only Save at the bottom.
