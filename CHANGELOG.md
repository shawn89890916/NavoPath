...
- 重构指标视图甜甜圈图外部标注为受约束的 rail layout：上一版自由 force layout 导致标签飞到页面边缘、压进右侧 summary 区、折线消失。现改为确定性双轨布局——左右两侧各一条固定 x 轨（railExtent=142），labelX 完全由 side 决定不漂移；仅 y 方向做单次排序+下推避让（minGap=26px），溢出时整列上移再 clamp 到 labelMinY/labelMaxY 边界内。引线恢复为三点式 anchor→elbow→水平段，elbow 在 visualOuter+40 处有清晰斜线段，水平段终止于 label 前 8px，线和文字形成一个整体 annotation。引线 1.75px、圆角、24% 半透明灰，激活时 50%；项目名 13px/600，激活时 650。hover 时仅当前扇区 anchor/elbow 跟随外径扩展，标签位置稳定不重排。
...
- Rebuilt the Metrics donut outside annotations as a constrained rail layout: the previous free-form force layout let labels fly to the page edge, overlap the right-side summary panel, and lose their leader lines. Now uses a deterministic two-rail layout — left and right sides each get a fixed x rail (railExtent=142), labelX is entirely side-driven and never drifts; only y gets a single sort-and-push collision pass (minGap=26px), with the whole lane shifting up if it overflows and then clamping within labelMinY/labelMaxY bounds. Leader lines restored to three-point anchor→elbow→horizontal-segment, with the elbow at visualOuter+40 giving a clear diagonal and the horizontal segment ending 8px before the label so line and text form one annotation unit. Leader stroke is 1.75px, rounded, 24% semi-transparent gray (50% when active); label typography 13px/600 (650 when active). On hover only the active segment's anchor/elbow follow the outer-radius expansion while label positions stay stable and don't reflow.
