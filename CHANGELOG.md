...
### 修复
- 修复连续触发不同方向的手动同步时，后一次推送或拉取请求可能被正在进行的同步吞掉的问题；不同方向现在会安全排队并依次完成。
...
### Fixed
- Fixed consecutive manual syncs in different directions dropping the later push or pull request. Different directions are now queued safely and completed in order.
