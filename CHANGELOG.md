...
- 重构指标视图甜甜圈图外部标注为 force-based annotation system：把每个项目标签抽象为轻量节点（含锚点、理想位置、实际位置、文本包围盒），运行 6 轮 spring-toward-target + 垂直碰撞避让 + 阻尼，让标签在同侧 rail 内柔和错开、不再互相遮挡，视觉上有类似 Obsidian 关系图谱标签的"settle"质感而非硬坐标堆放。引线改为两段式 anchor→elbow→label，斜线段缩短为 visualOuter+26 的短 tick，水平段终止于标签节点；hover 时仅当前扇区的 anchor/elbow 跟随外径扩展（88→94），标签位置保持稳定不跳变。引线加粗至 2.25px、圆角连接、22% 纸墨色半透明，激活时加深至 38%；项目名字号 11px/600，激活时填充加深。单一透明命中圆盘统一处理 hover，cursor 不再在元素间抽搐。
...
- Rebuilt the Metrics donut outside annotations as a force-based annotation system: each project label is now a lightweight node (with anchor, ideal position, real position, and text bounding box) that runs 6 iterations of spring-toward-target + vertical collision resolution + damping, so labels gently settle apart within their side rail without overlapping — giving a quiet Obsidian-graph-style "settle" feel rather than hard-coded coordinate dumps. Leader lines are now two-segment anchor→elbow→label with a short visualOuter+26 radial tick and a horizontal segment ending at the label node; on hover only the active segment's anchor/elbow follow the outer-radius expansion (88→94) while the label position stays stable. Leader stroke thickened to 2.25px with round joins and 22% paper-ink semi-transparent color (deepening to 38% when active); label typography is 11px/600 with a deeper fill when active. A single transparent hit disk handles all hover so the cursor no longer flickers between elements.
