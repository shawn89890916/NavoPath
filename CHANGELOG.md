...
- 时间占比统计的默认当天和最近 7、30、90 天边界现在按本地日历日期计算，不再因 UTC 转换把区间下界提前一天；跨午夜计划使用日历日跨度计算分钟数，经过夏令时切换时也不会少算或多算一小时。
...
- Time-share metrics now calculate the default current day and rolling 7-, 30-, and 90-day boundaries from local calendar dates instead of moving the lower bound back a day during UTC conversion. Planned time spanning midnight uses calendar-day distance, preventing daylight-saving transitions from adding or removing an hour.
